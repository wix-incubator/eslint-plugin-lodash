...
module.exports = function (context) {
    ...
    function isLodashCollectionFunction(node) {
        return esUtil.isLodashCall(node) && _.includes(matchesProperty, esUtil.getMethodName(node));
    }
    ...
    return {
        CallExpression: function (node) {
            try {
                ...
                } else if (_.includes(matchesProperty, esUtil.getMethodName(node)) && esUtil.isLodashWrapper(node.callee.object) && shouldNodePreferProp(node, 0)) {
                    context.report(node.callee.property, 'Prefer matches property syntax');
                }
            } catch (e) {
